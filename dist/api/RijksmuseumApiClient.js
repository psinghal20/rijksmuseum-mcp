...
import { AAT, RKM_DIMENSION_TYPES, } from '../types.js';
const CONCURRENCY_LIMIT = 10;
export class RijksmuseumApiClient {
    constructor() {
        this.BASE_URL = 'https://data.rijksmuseum.nl';
    ...
    async resolveImageUrl(obj) {
        try {
            ...
            const visualItem = await this.resolveEntity(visualItemRef.id);
            const digitalObjectRef = visualItem.digitally_shown_by?.[0];
            if (!digitalObjectRef?.id)
                return null;
            // Hop 3: Resolve the DigitalObject to get access_point (IIIF URL)
            const digitalObject = await this.resolveEntity(digitalObjectRef.id);
    ...
    static hasClassification(classifiedAs, aatUri) {
        return classifiedAs?.some(c => (typeof c === 'string' ? c : c.id) === aatUri) ?? false;
    }
    static languageMatches(langRefs, lang) {
        const langUri = lang === 'nl' ? AAT.LANG_NL : AAT.LANG_EN;
        return langRefs?.some(l => l.id === langUri) ?? false;
    }
    static extractObjectNumber(obj) {
        const entries = obj.identified_by ?? [];
        for (const entry of entries) {
            if (entry.type === 'Identifier' && RijksmuseumApiClient.hasClassification(entry.classified_as, AAT.OBJECT_NUMBER)) {
                return entry.content;
            }
        }
        return null;
    }
    static extractTitle(obj, lang = 'en') {
        const entries = obj.identified_by ?? [];
    ...
    static extractAllTitles(obj) {
        const entries = obj.identified_by ?? [];
        const names = entries.filter((e) => e.type === 'Name');
        return names.map(name => {
            let language = null;
            if (RijksmuseumApiClient.languageMatches(name.language, 'en'))
                language = 'en';
            else if (RijksmuseumApiClient.languageMatches(name.language, 'nl'))
                language = 'nl';
            let type = null;
            if (RijksmuseumApiClient.hasClassification(name.classified_as, AAT.TITLE))
                type = 'title';
            else if (RijksmuseumApiClient.hasClassification(name.classified_as, AAT.DISPLAY_TITLE))
                type = 'display_title';
            return { title: name.content, language, type };
        });
    }
    static extractArtist(obj, lang = 'en') {
    ...
    static extractDates(obj, lang = 'en') {
        const timespan = obj.produced_by?.timespan;
        if (!timespan)
            return { display: null, begin: null, end: null };
        // Extract display date from identified_by names on the timespan
        let display = timespan._label ?? null;
        if (!display && timespan.identified_by) {
            const names = timespan.identified_by.filter((e) => e.type === 'Name');
    ...
    static extractDescription(obj, lang = 'en') {
        const langUri = lang === 'nl' ? AAT.LANG_NL : AAT.LANG_EN;
        ...
        for (let i = 0; i < subjectOf.length; i++) {
            ...
            if (!('type' in entry) || entry.type !== 'LinguisticObject')
                continue;
            const lingObj = entry;
            if (!lingObj.part)
                continue;
            for (const part of lingObj.part) {
                if (!RijksmuseumApiClient.hasClassification(part.classified_as, AAT.DESCRIPTION))
                    continue;
                if (!part.content)
                    continue;
        ...
        for (const ref of refs) {
            if (ref.type === 'LinguisticObject' &&
                RijksmuseumApiClient.hasClassification(ref.classified_as, AAT.DESCRIPTION) &&
    ...
    static extractDimensions(obj) {
        ...
        return dims
            ...
            // Map unit
            let unit = 'unknown';
            if (d.unit?.id === AAT.UNIT_CM)
                unit = 'cm';
            else if (d.unit?.id === AAT.UNIT_KG)
                unit = 'kg';
            else if (d.unit?._label)
                unit = d.unit._label;
            return { type: dimType, value: d.value, unit };
        })
            .filter(d => d.type !== 'unknown');
    }
    static extractInscriptions(obj, lang = 'en') {
        const refs = obj.referred_to_by ?? [];
        const inscriptions = [];
    ...
    static extractMaterialRefs(obj) {
        // Return material entity IDs for async resolution
        return (obj.made_of ?? []).map(m => m.id).filter(Boolean);
    }
    async resolveMaterialNames(obj, lang = 'en') {
        const materialIds = RijksmuseumApiClient.extractMaterialRefs(obj);
        if (materialIds.length === 0)
            return [];
        const settled = await Promise.allSettled(materialIds.map(async (id) => {
            const material = await this.resolveEntity(id);
    ...
    static extractWebUrl(obj) {
        const subjectOf = obj.subject_of ?? [];
        for (const entry of subjectOf) {
            if ('classified_as' in entry && RijksmuseumApiClient.hasClassification(entry.classified_as, AAT.WEB_PAGE)) {
    ...
    async searchCollection(params) {
        ...
        if (params.objectNumber)
            apiParams.objectNumber = params.objectNumber;
        if (params.creator)
            apiParams.creator = params.creator;
        if (params.creationDate)
            apiParams.creationDate = params.creationDate;
        if (params.description)
            apiParams.description = params.description;
        if (params.type)
            apiParams.type = params.type;
        if (params.technique)
            apiParams.technique = params.technique;
        if (params.material)
            apiParams.material = params.material;
        if (params.aboutActor)
            apiParams.aboutActor = params.aboutActor;
        if (params.imageAvailable !== undefined)
            apiParams.imageAvailable = String(params.imageAvailable);
        if (params.pageToken)
    ...
    async searchArtworks(params) {
        ...
        for (let i = 0; i < refs.length; i += CONCURRENCY_LIMIT) {
            ...
            const settled = await Promise.allSettled(batch.map(async (ref) => {
                const obj = await this.resolveEntity(ref.id);
    ...
    async getArtistTimeline(artist, maxWorks = 10) {
        ...
        const timelineWorks = items.map(item => {
